...
<form>
    <div class="grid gap-6 mb-6 md:grid-cols-2">
        ...
        <div>
            ...
            <input type="number" id="company" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="5" required />
        </div>  
        <div>
            ...
            <select v-model="selectedAge" class="bg-gray-50 border cursor-pointer border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
               <option v-for="(item, index) in ages" :key="index" :value="item.value">
                  {{ item.age }}
               </option>
               
            </select>
        </div>
        <p class="text-white">{{ typeof selectedAge }} {{ selectedAge }}</p>
    </div>
